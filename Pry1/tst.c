...
int main(int argc, char *argv[]) {
    ...
    else printf("La cola de mensajes esta llena.\n");
    ...
    if (childID == 0){ // Padre
        ...
        msg.type = processes[childCounter];
        msg.filePosition = 0;
        msgsnd(msqid, (void *)&msg, sizeof(msg), 0);  // Manda a leer al primero, no espera porque es el primero

        do {
            printf("Entra al ciclo del padre\n");
            msgrcv(msqid, &msg, sizeof(msg), 1, 0);  // Ya leyo porque type=1
            //printf("type: %d, filePosition: %d\n", msg.type, msg.filePosition);
        ...
        while (1) {
            ...
            msgrcv(msqid, &msg, sizeof(msg), childID, 0);  // recibe tipo = su pid
            ...
            msgsnd(msqid, (void *)&msg, sizeof(msg), 0);  // Manda mensaje al padre para informar que termino de leer
            
            searchPattern(msqid);  // Procesa los bytesLeidos leidos
        }
    }
